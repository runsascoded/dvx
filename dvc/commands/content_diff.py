...
def _get_cache_path_for_ref(repo, dvc_path: str, ref: str | None) -> str | None:
    ...
    try:
        if ref:
            ...
            return os.path.join(
                repo.root_dir, ".dvc", "cache", "files", "md5", md5_base[:2], md5_base[2:] + suffix
            )
...
def _run_pipeline_diff(
    ...
    try:
        ...
        ui.error_write(
            "Pipeline diffing requires the 'dffs' package. Install with: pip install dffs"
        )
        return 1
...
def _show_summary(ctx, refspec, ref, targets):
    ...
    from dvc.commands.diff import CmdDiff
...
@click.option(
    "-c/-C", "--color/--no-color", default=None, help="Force or prevent colorized output."
)
...
@click.option(
    ...
    "--summary",
    is_flag=True,
    help="Show summary of changes (files and hashes) instead of content diff.",
)
...
@click.option(
    "-x", "--exec-cmd", "exec_cmds", multiple=True, help="Command to execute before diffing."
)
