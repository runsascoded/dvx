...
def _check_one_target(target, with_deps: bool) -> dict:
    ...
    from dvc.run.dvc_files import get_dvc_file_path, is_output_fresh, read_dvc_file
...
class CmdStatus(CmdBase):
    def run(self):
        ...
        with ThreadPoolExecutor(max_workers=max_workers, cancel_on_error=False) as executor:
            if show_progress:
                ui.write(
                    f"Checking {total} artifacts ({executor._max_workers} workers)...", stderr=True
                )
            ...
            for result in executor.imap_unordered(check_fn, targets):
                ...
                if show_progress:
                    ...
                    ui.write(
                        f"\rChecking artifacts: {completed}/{total} ({pct}%)", end="", stderr=True
                    )
