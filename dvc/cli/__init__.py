...
def parse_args(args=None):
    """Parse command line arguments using the argparse-based parser.

    This is a convenience function for tests and scripts that need to
    parse arguments without invoking the full CLI.
    """
    from .parser import get_main_parser

    return get_main_parser().parse_args(args)
...
def _log_exceptions(exc: Exception) -> int | None:
    ...
    if isinstance(exc, OSError):
        ...
        if exc.errno == errno.EMFILE:
            logger.exception(
                ("too many open files, please visit %s to see how to handle this problem"),
