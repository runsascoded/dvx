...
class DvxContext:
    ...
    def __init__(
        self, cd: str = ".", quiet: int = 0, verbose: int = 0, wait_for_lock: bool = False
    ):
        self.cd = cd
        self.quiet = quiet
...
@click.option(
    "-C", "--cd", default=".", help="Change to directory before executing.", metavar="<path>"
)
...
@click.version_option(__version__, "-V", "--version", prog_name="dvc")
@click.pass_context
...
@click.option(
    "--no-relink", "relink", flag_value=False, default=True, help="Don't recreate links from cache."
)
@click.pass_context
...
def status_cmd(ctx, targets, with_deps, jobs, verbose, as_json):
    ...
    from dvc.commands.status import _check_one_target
    from dvc.ui import ui
...
@click.option(
    "-p", "--projects", multiple=True, metavar="<path>", help="Additional projects to consider."
)
...
@click.pass_context
def push_cmd(
    ctx, targets, remote, all_branches, all_tags, all_commits, with_deps, recursive, jobs, glob
):
...
@click.pass_context
def fetch_cmd(
    ctx, targets, remote, all_branches, all_tags, all_commits, with_deps, recursive, jobs
):
...
def cache_path_cmd(ctx, target, rev, remote, absolute):
    ...
    from dvc.commands.cache import _get_output_from_target
    from dvc.repo import Repo
    from dvc.ui import ui
...
from dvc.commands.content_diff import diff  # noqa: E402
