...
def _hash_one_artifact(dvc_path: Path) -> dict:
    ...
    try:
        actual_hash, size, was_cached = get_artifact_hash_cached(output_path, compute_md5)
...
class CmdFsck(CmdBase):
    def run(self):
        from dvx.run.status import ArtifactStatusDB
        ...
        if args.json:
            ...
            if not args.quiet:
                ui.write("")
                ui.write(f"OK: {ok_count}, Mismatch: {mismatch_count}, Missing: {missing_count}, Error: {error_count}")
