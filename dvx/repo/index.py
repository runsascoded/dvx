...
from dvx.fs.callbacks import DEFAULT_CALLBACK
...
if TYPE_CHECKING:
    ...
    from dvx.dependency import Dependency
    from dvx.fs.callbacks import Callback
    from dvx.output import Output
    from dvx.repo import Repo
    from dvx.repo.stage import StageInfo
    from dvx.stage import Stage
    from dvx.types import TargetType
...
def _load_storage_from_out(storage_map, key, out):
    ...
    from dvx.cachemgr import LEGACY_HASH_NAMES
    from dvx.config import NoRemoteError
...
class Index:
    ...
    def outs_trie(self) -> "Trie":
        from dvx.repo.trie import build_outs_trie

        return build_outs_trie(self.stages)

    @cached_property
    def outs_graph(self) -> "DiGraph":
        from dvx.repo.graph import build_outs_graph

        return build_outs_graph(self.graph, self.outs_trie)

    @cached_property
    def graph(self) -> "DiGraph":
        from dvx.repo.graph import build_graph
    ...
    def collect_targets(
        ...
        from dvx.exceptions import DvcException
        from dvx.repo.stage import StageInfo
        from dvx.utils.collections import ensure_list
