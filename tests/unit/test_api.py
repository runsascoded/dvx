...
def test_open_raises_error_if_no_context(tmp_dir, dvc):
    ...
    fd = api.open("foo")
    with pytest.raises(AttributeError, match=re.escape("should be used in a with statement.")):
        fd.read()
...
def test_open_rev_raises_error_on_wrong_mode(tmp_dir, dvc):
    ...
    with pytest.raises(ValueError, match=re.escape("Only reading `mode` is supported.")):
